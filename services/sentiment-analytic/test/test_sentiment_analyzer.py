...
def test_get_entity_sentiments_postive_sentence(politicians: List[Politician], sentiment_analyzer: SentimentAnalyzer):
...
def test_get_entity_sentiments_negative_sentence(politicians: List[Politician], sentiment_analyzer: SentimentAnalyzer):
    sentence = 'John Smith is terrible'
    predictions = sentiment_analyzer.get_entity_sentiments([sentence], [politicians])
    assert predictions[0][2] < 0


def test_get_entity_sentiments_subject_results(politicians: List[Politician], sentiment_analyzer: SentimentAnalyzer):
...
def test_get_entity_sentiments_non_specific_subject(politicians: List[Politician], sentiment_analyzer: SentimentAnalyzer):
